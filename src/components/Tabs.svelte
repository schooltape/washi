    ...
    <Tabs.List
      ...
      {#each Object.entries(items) as [id, label]}
        <Tabs.Trigger
          {...triggerProps}
          value={id}
          class="h-8 flex-grow rounded-md border-1 border-ctp-surface0 bg-ctp-base px-2 py-2 transition-colors duration-300 data-[state=active]:border-(--ctp-accent) data-[state=active]:bg-(--ctp-accent)/20 data-[state=active]:text-ctp-text"
          >{label}</Tabs.Trigger>
      {/each}
    </Tabs.List>
  {/if}
